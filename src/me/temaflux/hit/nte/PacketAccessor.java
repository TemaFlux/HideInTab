    ...
    static {
        ...
        try {
            ...
            for (PacketData packetData : PacketData.values())
                if (version.contains(packetData.name())) currentVersion = packetData;

            if (CAULDRON_SERVER) currentVersion = PacketData.cauldron;

            if (currentVersion != null) {
                PREFIX = getNMS(currentVersion.prefix);
                SUFFIX = getNMS(currentVersion.suffix);
                MEMBERS = getNMS(currentVersion.members);
                TEAM_NAME = getNMS(currentVersion.teamName);
                PARAM_INT = getNMS(currentVersion.paramInt);
                PACK_OPTION = getNMS(currentVersion.packOption);
                DISPLAY_NAME = getNMS(currentVersion.displayName);

                if (!isLegacyVersion()) TEAM_COLOR = getNMS(currentVersion.color);

                if (isPushVersion(version)) PUSH = getNMS(currentVersion.push);

                if (isVisibilityVersion(version)) VISIBILITY = getNMS(currentVersion.visibility);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
